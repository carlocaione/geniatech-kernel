...
/* WIFI&BT CLOCK ON Flag */
static int WIFI_BT_CLOCK_ON = 0;
/* BT ON Flag */
static int BT_ON = 0;
...
{
	DBG_LINE_INFO();
    gpio_set_status(PAD_GPIOD_5,gpio_status_out);//set wifi_en gpio mode out
    if(is_on){
        if((!WIFI_ON) && (!BT_ON)){
...
void extern_wifi_reset(int is_on)
{
	...
	/* } */
	printk("wifi reset but do nothing now! \n");
    return;
}
...
static void reg_on_control(int is_on)
{
   ...
   //set wifi_en
    ...
    if(is_on){
        ...
        /* only pull down reg_on pin when wifi and bt off */
        /* if((!WIFI_ON) && (!BT_ON)){ */
		gpio_out(PAD_GPIOD_5,1);
...
void wifi_dev_init(void)
{
	...
	/* wifi_gpio_init(); */
#if 1
	extern_wifi_set_enable(0);//ainou for wl_reg_on
	udelay(400);
	extern_wifi_set_enable(1);//ainou for wl_reg_on
#endif
}
...
{
#define USB_B_POW_GPIO  PREG_EGPIO
#define USB_B_POW_GPIO_BIT  24
...
static void hdmi_5v_ctrl(unsigned int pwr)
{
    if(pwr){
        ...
        aml_clr_reg32_mask(P_PREG_PAD_GPIO2_O,(1<<14));
        aml_clr_reg32_mask(P_PREG_PAD_GPIO2_EN_N,(1<<14));
    }
    else{
        printk("HDMI 5V Power Off\n");
        aml_clr_reg32_mask(P_PREG_PAD_GPIO2_O,(0<<14));
        aml_set_reg32_mask(P_PREG_PAD_GPIO2_EN_N,(1<<14));
    }
}
...
static unsigned int vcck_pwm_table[MESON_CS_MAX_STEPS] = {
	0x01001b, 0x01001b, 0x02001a, 0x02001a, 
	0x050017, 0x050017, 0x090013, 0x090013, 
	0x0e000e, 0x0e000e, 0x0e000e, 0x0e000e, 
	0x130009, 0x130009, 0x130009, 0x130009,  
};
...
static void dummy_codec_device_init(void)
{
    ...
    gpio_out(PAD_GPIOD_9, 0); //set mute as init
}
