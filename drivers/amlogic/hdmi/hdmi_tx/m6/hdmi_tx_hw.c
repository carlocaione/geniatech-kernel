...
static 
#endif
void hdmi_non_standard_tvenc_set(Hdmi_tx_video_para_t *param)
{
    unsigned long VFIFO2VD_TO_HDMI_LATENCY = 2; // Annie 01Sep2011: Change value from 3 to 2, due to video encoder path delay change.
    unsigned long TOTAL_PIXELS, PIXEL_REPEAT_HDMI, PIXEL_REPEAT_VENC, ACTIVE_PIXELS;
    unsigned FRONT_PORCH, HSYNC_PIXELS, ACTIVE_LINES, INTERLACE_MODE, TOTAL_LINES, SOF_LINES, VSYNC_LINES;
    ...
    HDMI_DEBUG();
    aml_write_reg32(P_HHI_VID_DIVIDER_CNTL, 0x11943);
    ...
    switch(param->VIC){
        case HDMI_640x480p60:
            ...
            break;
        case HDMI_800x600:
            aml_write_reg32(P_HHI_VID_PLL_CNTL, 0x40421);
            aml_write_reg32(P_HHI_HDMI_CLK_CNTL, 0x100);
            break;
        case HDMI_1024x768:
            aml_write_reg32(P_HHI_VID_PLL_CNTL, 0x40436);
            aml_write_reg32(P_HHI_HDMI_CLK_CNTL, 0x100);
            break;
        case HDMI_1280_1024:
            break;
        case HDMI_1440x900:
            aml_write_reg32(P_HHI_VID_PLL_CNTL, 0x425);
            aml_write_reg32(P_HHI_HDMI_CLK_CNTL, 0x100);
            break;     
        case HDMI_1680x1050:
            aml_write_reg32(P_HHI_VID_PLL_CNTL, 0x432);
            aml_write_reg32(P_HHI_HDMI_CLK_CNTL, 0x100);
            break;  
        default:
            break;
    ...
    else if(param->VIC==HDMI_800x600){
         INTERLACE_MODE     = 0;                   
         PIXEL_REPEAT_VENC  = 0;                   
         PIXEL_REPEAT_HDMI  = 0;                   
         ACTIVE_PIXELS      = (800*(1+PIXEL_REPEAT_HDMI)); // Number of active pixels per line.
         ACTIVE_LINES       = (600/(1+INTERLACE_MODE));    // Number of active lines per field.
         LINES_F0           = 628;                 
         LINES_F1           = 628;                 
         FRONT_PORCH        = 40;                  
         HSYNC_PIXELS       = 128;                  
    ...
    else if(param->VIC==HDMI_1024x768){
         INTERLACE_MODE     = 0;                   
         PIXEL_REPEAT_VENC  = 0;                   
         PIXEL_REPEAT_HDMI  = 0;                   
    ...
    else if(param->VIC==HDMI_1280_1024){
         INTERLACE_MODE     = 0;                   
         PIXEL_REPEAT_VENC  = 0;                   
         PIXEL_REPEAT_HDMI  = 0;                   
    ...
    else if(param->VIC==HDMI_1440x900){
         INTERLACE_MODE      =0;              
         PIXEL_REPEAT_VENC   =0;              
         PIXEL_REPEAT_HDMI   =0;              
         ACTIVE_PIXELS       =(1440*(1+PIXEL_REPEAT_HDMI)); // Number of active pixels per line.
         ACTIVE_LINES        =(900/(1+INTERLACE_MODE));    // Number of active lines per field.
         LINES_F0            =926;           
         LINES_F1            =926;           
         FRONT_PORCH         =48;             
         HSYNC_PIXELS        =32;             
    ...
    else if(param->VIC==HDMI_1680x1050){
         INTERLACE_MODE      =0;              
         PIXEL_REPEAT_VENC   =0;              
         PIXEL_REPEAT_HDMI   =0;              
         ACTIVE_PIXELS       =(1680*(1+PIXEL_REPEAT_HDMI)); // Number of active pixels per line.
         ACTIVE_LINES        =(1050/(1+INTERLACE_MODE));    // Number of active lines per field.
         LINES_F0            =1080;           
         LINES_F1            =1080;           
         FRONT_PORCH         =48;             
         HSYNC_PIXELS        =32;             
    ...
    if(new_reset_sequence_flag==0){
        if(serial_reg_val==0){
            if((param->VIC==HDMI_1080p30)||(param->VIC==HDMI_720p60)||(param->VIC==HDMI_1080i60)
                ||(param->VIC==HDMI_1080p24) || (param->VIC==HDMI_640x480p60) || (param->VIC==HDMI_1024x768)
                ||(param->VIC==HDMI_1280_1024)|| (param->VIC==HDMI_1440x900) || (param->VIC==HDMI_1680x1050)){
...
//            }
        ...
        case HDMI_640x480p60:
            ...
            break;
        case HDMI_800x600:
            set_vmode_clk(VMODE_SVGA);
            break;
        case HDMI_1024x768:
            set_vmode_clk(VMODE_XGA);
            break;
        case HDMI_1280_1024:
            set_vmode_clk(VMODE_SXGA);
            break;
        case HDMI_1440x900:
            set_vmode_clk(VMODE_VGA_1440x900);
            break;
        case HDMI_1680x1050:
            set_vmode_clk(VMODE_VGA_1680x1050);
            break;
        case HDMI_1080i60:
        case HDMI_1080i50:
...
static int hdmitx_m3_set_dispmode(hdmitx_dev_t* hdmitx_device, Hdmi_tx_video_para_t *param)
{
    if(param == NULL){ //disable HDMI
        ...
        &&(param->VIC!=HDMI_640x480p60)&&(param->VIC!=HDMI_800x600)
        &&(param->VIC!=HDMI_1024x768)&&(param->VIC!=HDMI_1280_1024)
        &&(param->VIC!=HDMI_1440x900)&&(param->VIC!=HDMI_1680x1050)
    ...
    switch(param->VIC){
        ...
        case HDMI_1440x900:
        case HDMI_1680x1050:
            hdmi_non_standard_tvenc_set(param);
            break;
        default:
