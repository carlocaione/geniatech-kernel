...
void  change_vdac_setting(unsigned int  vdec_setting,vmode_t  mode)
{
	...
	if (mode >= VMODE_VGA && mode <= VMODE_WUXGA){
		aml_set_reg32_bits(P_ENCP_DACSEL_0,3, 0, 4);
		aml_set_reg32_bits(P_ENCP_DACSEL_0,4, 4, 4);
		aml_set_reg32_bits(P_ENCP_DACSEL_0,5, 8, 4);
	}else{
		for(i=0;i<3;i++)
		{
...
int tvoutc_setclk(tvmode_t mode)
{
	...
	switch(mode)
	{
		...
		case TVMODE_SXGA:
		case TVMODE_WXGA:
		case TVMODE_VGA_1440x900:
        case TVMODE_VGA_1680x1050:
		case TVMODE_WUXGA:
			  break;
		default:
...
static void tvoutc_set_pinmux(tvmode_t mode)
{
	if( (mode >= TVMODE_VGA) && (mode <=TVMODE_WUXGA))
	{
		printk("Set VGA HSync VSync PINMUX\n");
		//Set VGA HSync VSync PINMUX
#ifdef CONFIG_ARCH_MESON1

#else
		CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_2,(3<<0));  //clear PWMA,PWMB
		CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_1,(3<<26));  //clear LCD_VGHL_PWM,LED_BL_PWM
		SET_CBUS_REG_MASK(PERIPHS_PIN_MUX_0, (3<<20));  //set VGA_HS, VGA_VS
#endif
	}
	else
	{
...
#else
		CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_0,(3<<20));
#endif
	}
}
