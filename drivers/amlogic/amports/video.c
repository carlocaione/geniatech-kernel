...
static void vpp_settings_h(vpp_frame_par_t *framePtr)
{
    ...
    WRITE_MPEG_REG(VPP_POSTBLEND_VD1_H_START_END + cur_dev->vpp_off,
                   ((framePtr->VPP_hsc_startp & VPP_VD_SIZE_MASK) << VPP_VD1_START_BIT) |
                   ((framePtr->VPP_hsc_endp   & VPP_VD_SIZE_MASK) << VPP_VD1_END_BIT));

    WRITE_MPEG_REG(VPP_BLEND_VD2_H_START_END + cur_dev->vpp_off,
                   ((framePtr->VPP_hsc_startp & VPP_VD_SIZE_MASK) << VPP_VD1_START_BIT) |
                   ((framePtr->VPP_hsc_endp   & VPP_VD_SIZE_MASK) << VPP_VD1_END_BIT));

    WRITE_MPEG_REG(VPP_HSC_REGION12_STARTP + cur_dev->vpp_off,
                   (0 << VPP_REGION1_BIT) |
                   ((r1 & VPP_REGION_MASK) << VPP_REGION2_BIT));

    WRITE_MPEG_REG(VPP_HSC_REGION34_STARTP + cur_dev->vpp_off,
                   ((r2 & VPP_REGION_MASK) << VPP_REGION3_BIT) |
                   ((r3 & VPP_REGION_MASK) << VPP_REGION4_BIT));
    WRITE_MPEG_REG(VPP_HSC_REGION4_ENDP + cur_dev->vpp_off, r3);

    WRITE_MPEG_REG(VPP_HSC_START_PHASE_STEP + cur_dev->vpp_off,
    ...
    WRITE_MPEG_REG(VPP_LINE_IN_LENGTH + cur_dev->vpp_off, framePtr->VPP_line_in_length_);
    WRITE_MPEG_REG(VPP_PREBLEND_H_SIZE + cur_dev->vpp_off, framePtr->VPP_line_in_length_);
}
...
static void vpp_settings_v(vpp_frame_par_t *framePtr)
{
    ...
    WRITE_MPEG_REG(VPP_POSTBLEND_VD1_V_START_END + cur_dev->vpp_off,
    ...
    WRITE_MPEG_REG(VPP_BLEND_VD2_V_START_END + cur_dev->vpp_off,
                   (((framePtr->VPP_vsc_endp / 2) & VPP_VD_SIZE_MASK) << VPP_VD1_START_BIT) |
                   (((framePtr->VPP_vsc_endp) & VPP_VD_SIZE_MASK) << VPP_VD1_END_BIT));

    WRITE_MPEG_REG(VPP_VSC_REGION12_STARTP + cur_dev->vpp_off, 0);
    WRITE_MPEG_REG(VPP_VSC_REGION34_STARTP + cur_dev->vpp_off,
                   ((r & VPP_REGION_MASK) << VPP_REGION3_BIT) |
                   ((r & VPP_REGION_MASK) << VPP_REGION4_BIT));
    WRITE_MPEG_REG(VPP_VSC_REGION4_ENDP + cur_dev->vpp_off, r);

    WRITE_MPEG_REG(VPP_VSC_START_PHASE_STEP + cur_dev->vpp_off,
                   vpp_filter->vpp_vsc_start_phase_step);
}

static void zoom_display_horz(void)
{
    WRITE_MPEG_REG(VD1_IF0_LUMA_X0 + cur_dev->viu_off,
                   (zoom_start_x_lines << VDIF_PIC_START_BIT) |
                   (zoom_end_x_lines   << VDIF_PIC_END_BIT));

    WRITE_MPEG_REG(VD1_IF0_CHROMA_X0 + cur_dev->viu_off,
                   (zoom_start_x_lines / 2 << VDIF_PIC_START_BIT) |
                   (zoom_end_x_lines / 2   << VDIF_PIC_END_BIT));

    WRITE_MPEG_REG(VD1_IF0_LUMA_X1 + cur_dev->viu_off,
                   (zoom_start_x_lines << VDIF_PIC_START_BIT) |
                   (zoom_end_x_lines   << VDIF_PIC_END_BIT));

    WRITE_MPEG_REG(VD1_IF0_CHROMA_X1 + cur_dev->viu_off,
                   (zoom_start_x_lines / 2 << VDIF_PIC_START_BIT) |
                   (zoom_end_x_lines / 2   << VDIF_PIC_END_BIT));

    WRITE_MPEG_REG(VIU_VD1_FMT_W + cur_dev->viu_off,
    ...
    WRITE_MPEG_REG(VIU_VD2_FMT_W + cur_dev->viu_off,
                   ((zoom_end_x_lines - zoom_start_x_lines + 1) << VD1_FMT_LUMA_WIDTH_BIT) |
                   ((zoom_end_x_lines / 2 - zoom_start_x_lines / 2 + 1) << VD1_FMT_CHROMA_WIDTH_BIT));
}
...
static void zoom_display_vert(void)
{
    if ((cur_dispbuf) && (cur_dispbuf->type & VIDTYPE_MVC)) {
        WRITE_MPEG_REG(VD1_IF0_LUMA_Y0 + cur_dev->viu_off,
                       (zoom_start_y_lines * 2 << VDIF_PIC_START_BIT) |
                       (zoom_end_y_lines * 2   << VDIF_PIC_END_BIT));

        WRITE_MPEG_REG(VD1_IF0_CHROMA_Y0 + cur_dev->viu_off,
        ...
        WRITE_MPEG_REG(VD1_IF0_LUMA_Y1 + cur_dev->viu_off,
                       (zoom_start_y_lines << VDIF_PIC_START_BIT) |
                       (zoom_end_y_lines << VDIF_PIC_END_BIT));

        WRITE_MPEG_REG(VD1_IF0_CHROMA_Y1 + cur_dev->viu_off,
                       ((zoom_start_y_lines / 2) << VDIF_PIC_START_BIT) |
                       ((zoom_end_y_lines / 2) << VDIF_PIC_END_BIT));
    }
}
...
static void vsync_toggle_frame(vframe_t *vf)
{
    ...
    WRITE_MPEG_REG(VD1_IF0_CANVAS0 + cur_dev->viu_off, disp_canvas[0]);
    WRITE_MPEG_REG(VD1_IF0_CANVAS1 + cur_dev->viu_off, disp_canvas[1]);
...
static void viu_set_dcu(vpp_frame_par_t *frame_par, vframe_t *vf)
{
    ...
    WRITE_MPEG_REG(VD1_IF0_GEN_REG + cur_dev->viu_off, r);
    WRITE_MPEG_REG(VD2_IF0_GEN_REG, r);

#if MESON_CPU_TYPE >= MESON_CPU_TYPE_MESON6
    if (vf->type & VIDTYPE_VIU_NV21) {
        WRITE_MPEG_REG_BITS(VD1_IF0_GEN_REG2 + cur_dev->viu_off, 1,0,1);
    } else {
        WRITE_MPEG_REG_BITS(VD1_IF0_GEN_REG2 + cur_dev->viu_off, 0,0,1);
    }
#endif

    /* chroma formatter */
    if (vf->type & VIDTYPE_VIU_444) {
        WRITE_MPEG_REG(VIU_VD1_FMT_CTRL + cur_dev->viu_off, HFORMATTER_YC_RATIO_1_1);
        WRITE_MPEG_REG(VIU_VD2_FMT_CTRL + cur_dev->viu_off, HFORMATTER_YC_RATIO_1_1);
        ...
        WRITE_MPEG_REG(VIU_VD1_FMT_CTRL + cur_dev->viu_off,
                       HFORMATTER_YC_RATIO_2_1 | HFORMATTER_EN |
                       VFORMATTER_RPTLINE0_EN | vini_phase | vphase | VFORMATTER_EN);

        WRITE_MPEG_REG(VIU_VD2_FMT_CTRL + cur_dev->viu_off,
                       HFORMATTER_YC_RATIO_2_1 | HFORMATTER_EN |
                       VFORMATTER_RPTLINE0_EN | vini_phase | vphase | VFORMATTER_EN);
    ...
    WRITE_MPEG_REG(VD1_IF0_RPT_LOOP + cur_dev->viu_off,
    ...
    WRITE_MPEG_REG(VD1_IF0_LUMA0_RPT_PAT + cur_dev->viu_off,   pat);
    WRITE_MPEG_REG(VD1_IF0_CHROMA0_RPT_PAT + cur_dev->viu_off, pat);
    WRITE_MPEG_REG(VD1_IF0_LUMA1_RPT_PAT + cur_dev->viu_off,   pat);
    WRITE_MPEG_REG(VD1_IF0_CHROMA1_RPT_PAT + cur_dev->viu_off, pat);
    ...
    if (((vf->type & VIDTYPE_INTERLACE) == 0) &&
        ...
        WRITE_MPEG_REG(VD1_IF0_LUMA_PSEL + cur_dev->viu_off,
                       (2 << 26) |    /* two pic mode */
                       (2 << 24) |    /* use own last line */
                       (2 << 8)  |    /* toggle pic 0 and 1, use pic0 first */
                       (0x01));       /* loop pattern */
        WRITE_MPEG_REG(VD1_IF0_CHROMA_PSEL + cur_dev->viu_off,
                       (2 << 26) |    /* two pic mode */
                       (2 << 24) |    /* use own last line */
                       (2 << 8)  |    /* toggle pic 0 and 1, use pic0 first */
                       (0x01));       /* loop pattern */
    } else {
        WRITE_MPEG_REG(VD1_IF0_LUMA_PSEL + cur_dev->viu_off, 0);
        WRITE_MPEG_REG(VD1_IF0_CHROMA_PSEL + cur_dev->viu_off, 0);
...
SET_FILTER:
    ...
    if (cur_dispbuf) {
        ...
        WRITE_MPEG_REG(VPP_VSC_INI_PHASE + cur_dev->vpp_off, ((u32)(vphase->phase) << 8));

        if (vphase->repeat_skip >= 0) {
            /* skip lines */
            WRITE_MPEG_REG_BITS(VPP_VSC_PHASE_CTRL + cur_dev->vpp_off,
            ...
            /* repeat first line */
            WRITE_MPEG_REG_BITS(VPP_VSC_PHASE_CTRL + cur_dev->vpp_off, 4,
                                VPP_PHASECTL_INIRCVNUMT_BIT,
                                VPP_PHASECTL_INIRCVNUM_WID);
            WRITE_MPEG_REG_BITS(VPP_VSC_PHASE_CTRL + cur_dev->vpp_off,
    ...
    if (((frame_par_ready_to_set) || (frame_par_force_to_set)) &&
        ...
        WRITE_MPEG_REG_BITS(VPP_SC_MISC + cur_dev->vpp_off,
                            vpp_filter->vpp_horz_coeff[0],
                            VPP_SC_HBANK_LENGTH_BIT,
                            VPP_SC_BANK_LENGTH_WID);

        if (vpp_filter->vpp_horz_coeff[1] & 0x8000) {
            WRITE_MPEG_REG(VPP_SCALE_COEF_IDX + cur_dev->vpp_off, VPP_COEF_HORZ | VPP_COEF_9BIT);
        } else {
            WRITE_MPEG_REG(VPP_SCALE_COEF_IDX + cur_dev->vpp_off, VPP_COEF_HORZ);
        }

        for (i = 0; i < (vpp_filter->vpp_horz_coeff[1] & 0xff); i++) {
            WRITE_MPEG_REG(VPP_SCALE_COEF + cur_dev->vpp_off, vpp_filter->vpp_horz_coeff[i + 2]);
        }
        ...
        WRITE_MPEG_REG(VPP_SCALE_COEF_IDX + cur_dev->vpp_off, VPP_COEF_VERT);
        for (i = 0; i < vpp_filter->vpp_vert_coeff[1]; i++) {
            WRITE_MPEG_REG(VPP_SCALE_COEF + cur_dev->vpp_off,
                           vpp_filter->vpp_vert_coeff[i + 2]);
        }

        WRITE_MPEG_REG(VPP_PIC_IN_HEIGHT + cur_dev->vpp_off,
                       cur_frame_par->VPP_pic_in_height_);

        WRITE_MPEG_REG_BITS(VPP_HSC_PHASE_CTRL + cur_dev->vpp_off,
        ...
        WRITE_MPEG_REG(VPP_POSTBLEND_H_SIZE + cur_dev->vpp_off, cur_frame_par->VPP_post_blend_h_size_);
